...
def _empaque_explicito_sin_cantidad_fila(prod):
    """
    Extrae empaque sólo de campos que realmente describen presentación/empaque.
    NO usa raw_row_text, porque allí también vive la columna CANTIDAD y puede
    confundirse qty=24 con pack=24.
    """
    if not isinstance(prod, dict):
        return 1

    textos = [
        str(prod.get("package_evidence_text") or ""),
        str(prod.get("package_text") or ""),
        str(prod.get("presentation") or ""),
        str(prod.get("size_text") or ""),
        str(prod.get("nombre_original_lectura") or ""),
        str(prod.get("nombre") or ""),
    ]
    for txt in textos:
        emp = _extraer_empaque_desde_tamano(txt)
        if emp > 1:
            return int(emp)
    return 1
...
def _linea_recibo_demuestra_unidad_fisica(prod):
    ...
    # Sólo evidencia explícita de presentación bloquea emp=1.
    # raw_row_text NO se usa aquí porque contiene también la cantidad facturada.
    if _empaque_explicito_sin_cantidad_fila(prod) > 1:
        return False
...
def _vision_pack_sospechoso_igual_cantidad(prod, emp_api):
    ...
    # Si una presentación/empaque EXPLÍCITO demuestra el pack, no es sospechoso.
    # No usar raw_row_text aquí: contiene la columna cantidad.
    if _empaque_explicito_sin_cantidad_fila(prod) > 1:
        return False
...
def _inferir_empaque_universal(prod, proveedor=""):
    ...
    # demuestra que q ya son unidades físicas, NO convertir q en pack.
    emp_evidencia = _extraer_empaque_desde_tamano(evidencia)
    try:
        q_actual = float(prod.get("cant") or 0)
    except Exception:
        q_actual = 0.0

    raw_pack_es_cantidad = (
        emp_evidencia > 1
        and q_actual >= 6
        and abs(float(emp_evidencia) - q_actual) < 1e-9
        and _linea_recibo_demuestra_unidad_fisica(prod)
    )

    if emp_evidencia > 1 and not es_deposito and not raw_pack_es_cantidad:
        return int(emp_evidencia), "evidencia_literal_fila", 92, False
...
def _resolver_costo_neto_linea_vision(item, cantidad, unidades_empaque):
    ...
    R30.3 corrige un error estructural:
    Vision puede colocar un TOTAL DE LÍNEA en un campo llamado
    net_price_per_package / price_unit_per_package. Si luego se multiplica
    por cantidad, el costo queda inflado.

    Reglas universales:
    - precio neto por empaque <= precio lista, salvo tolerancia de redondeo;
    - si hay descuento, precio neto debe aproximarse a lista - descuento;
    - total neto + ITBIS ≈ total con impuesto cuando esos campos existen;
    - si un supuesto "precio por empaque" coincide con un total de línea
      y cantidad > 1, se considera posible columna desplazada;
    - elegir el candidato respaldado por más relaciones contables.
    """
    adv = []
    ...
    # Precio neto esperado desde lista/descuento.
    neto_lista = None
    if list_price is not None and list_price > 0:
    ...
    # Total neto contable derivado de total con ITBIS.
    neto_desde_total = None
    if gross_line is not None and gross_line > 0 and tax_value is not None and tax_value >= 0:
        candidato = gross_line - tax_value
        if candidato > 0:
            neto_desde_total = candidato

    # Totales explícitos visibles.
    totales_visibles = [
        ("line_cost_net", line_cost),
        ("subtotal_net", subtotal_net),
        ("total_menos_itbis", neto_desde_total),
    ]
    totales_visibles = [(f, v) for f, v in totales_visibles if v is not None and v > 0]

    # Detectar campos "por empaque" sospechosos.
    def precio_por_empaque_confiable(nombre, precio):
        if precio is None or precio <= 0:
            return False, "vacío"

        # Neto pre-ITBIS no debe ser mucho mayor que precio lista.
        if list_price is not None and list_price > 0 and precio > list_price * 1.03:
            return False, "supera_precio_lista"

        # Con descuento conocido, el neto debe aproximarse al neto calculado.
        if neto_lista is not None and neto_lista > 0:
            if not cercanos(precio, neto_lista, rel=0.025, abs_min=0.25):
                # price_unit puede ser lista, pero net_price/unit_cost no deberían divergir tanto.
                if nombre in ("net_price_per_package", "unit_cost_net"):
                    return False, "no_cuadra_lista_descuento"

        # Si q>1 y este supuesto precio coincide con un total de línea,
        # es probable que Vision haya desplazado una columna.
        if q > 1:
            for fuente_total, total in totales_visibles:
                if cercanos(precio, total, rel=0.008):
                    return False, f"parece_{fuente_total}"

        return True, "ok"

    candidatos = []

    def agregar(valor, fuente, score, detalle=""):
    ...
    # A) Total con ITBIS - ITBIS: evidencia contable muy fuerte.
    if neto_desde_total is not None:
        score = 170
        if line_cost is not None and cercanos(line_cost, neto_desde_total):
            score += 25
        if subtotal_net is not None and cercanos(subtotal_net, neto_desde_total):
            score += 20
        agregar(neto_desde_total, "total_menos_itbis", score)

    # B) Precio lista menos descuento × cantidad.
    if q > 0 and neto_lista is not None and neto_lista > 0:
        esperado = q * neto_lista
        score = 145
        if line_cost is not None and cercanos(line_cost, esperado):
            score += 30
        if subtotal_net is not None and cercanos(subtotal_net, esperado):
            score += 25
        if neto_desde_total is not None and cercanos(neto_desde_total, esperado):
            score += 35
        agregar(esperado, "lista_descuento_x_cantidad", score)

    # C) Precios por empaque transcritos.
    precios = [
        ("net_price_per_package", net_pkg, 140),
        ("unit_cost_net", unit_cost, 135),
        ("price_unit_per_package", price_unit_package, 120),
    ]

    for nombre_precio, precio, base_score in precios:
        ok, motivo = precio_por_empaque_confiable(nombre_precio, precio)
        if not ok:
            if precio is not None and precio > 0:
                adv.append(
                    f"{nombre_precio} descartado como precio por empaque: {motivo}"
                )
            continue

        esperado = q * precio if q > 0 else None
        score = base_score

        if line_cost is not None and esperado is not None and cercanos(line_cost, esperado):
            score += 35
        if subtotal_net is not None and esperado is not None and cercanos(subtotal_net, esperado):
            score += 30
        if neto_desde_total is not None and esperado is not None and cercanos(neto_desde_total, esperado):
            score += 40
        if neto_lista is not None and cercanos(precio, neto_lista, rel=0.025, abs_min=0.25):
            score += 25

        agregar(esperado, f"{nombre_precio}_x_cantidad", score)

    # D) Totales explícitos como candidatos, pero no ganan solos frente
    # a una relación precio×cantidad validada.
    if line_cost is not None and line_cost > 0:
        score = 100
        if tax_value is not None and cercanos(line_cost, tax_value, rel=0.01):
            score -= 100
            adv.append("line_cost_net parecía contener ITBIS")
        if neto_desde_total is not None and cercanos(line_cost, neto_desde_total):
            score += 50
        if q > 0 and neto_lista is not None and cercanos(line_cost, q * neto_lista):
            score += 45
    ...
    if subtotal_net is not None and subtotal_net > 0:
        ...
        if tax_value is not None and cercanos(subtotal_net, tax_value, rel=0.01):
            score -= 100
            adv.append("subtotal_net parecía contener ITBIS")
        if neto_desde_total is not None and cercanos(subtotal_net, neto_desde_total):
            score += 50
        if q > 0 and neto_lista is not None and cercanos(subtotal_net, q * neto_lista):
            score += 45
    ...
    # Agrupar candidatos cercanos y premiar consenso.
    for i, c in enumerate(candidatos):
        consenso = 0
        for j, d in enumerate(candidatos):
            if i == j:
                continue
            if cercanos(c["valor"], d["valor"], rel=0.012, abs_min=0.25):
                consenso += 1
        c["score"] += consenso * 18
    ...
    # Confianza del costo para la barrera canónica.
    segunda = candidatos[1] if len(candidatos) > 1 else None
    diferencia_score = mejor["score"] - (segunda["score"] if segunda else 0)
    item["_costo_resuelto_score"] = float(mejor["score"])
    item["_costo_resuelto_margen"] = float(diferencia_score)
...
def _normalizar_resultado_vision_factura(data, nombre_archivo=""):
    ...
    for indice, item in enumerate(productos_api, start=1):
        ...
        p = {
            ...
            "costo_resuelto_score": item.get("_costo_resuelto_score", 0),
            "costo_resuelto_margen": item.get("_costo_resuelto_margen", 0),
            "net_price_per_package": item.get("net_price_per_package"),
            "unit_cost_net": item.get("unit_cost_net"),
...
def _detectar_costos_atipicos_origen():
    """
    Diagnóstico solamente. No sustituye costos por comparación entre productos.

    Para el MISMO código/producto, si diferentes compras tienen costo físico
    muy disperso, devuelve una alerta para revisar el origen monetario.
    """
    alertas = []
    for codigo, apariciones in (st.session_state.get("origen_productos_facturas", {}) or {}).items():
        ...
        for item in apariciones or []:
            try:
                ...
                total = float(item.get("costo_total") or 0)
            except Exception:
                continue
            if unidades <= 0 or total <= 0:
                continue
            filas.append((item, total / unidades))

        if len(filas) < 2:
            continue

        costos = sorted(c for _, c in filas if c > 0)
        if not costos:
            continue

        mediana = costos[len(costos)//2]
        if mediana <= 0:
            continue

        for item, costo in filas:
            ratio = costo / mediana
            if ratio < 0.65 or ratio > 1.55:
                alertas.append({
                    ...
                    "Costo físico": round(costo, 4),
                    "Referencia mediana": round(mediana, 4),
                    "Fuente costo": item.get("fuente_costo", ""),
                    "Estado": "REVISAR COSTO",
                })
    return alertas
...
def construir_df_productos():
    ...
    for codigo_original, data in st.session_state.inventario_acumulado.items():
        ...
        if clave in consolidados:
            ...
            actual["cantidad_facturada"] += float(data.get("cantidad_facturada", 0))
            actual["unidades_fisicas"] += float(data.get("unidades_fisicas", data.get("stock", 0)))

            for _emp_obs in (data.get("empaques_observados") or [data.get("emp", 1)]):
                try:
                    _emp_obs = int(float(_emp_obs or 1))
                except Exception:
                    _emp_obs = 1
                if _emp_obs not in actual["empaques_observados"]:
                    actual["empaques_observados"].append(_emp_obs)

            for _udm_obs in (data.get("udm_observadas") or [data.get("udm_factura", "")]):
                _udm_obs = str(_udm_obs or "").strip()
                if _udm_obs and _udm_obs not in actual["udm_observadas"]:
                    actual["udm_observadas"].append(_udm_obs)
            ...
            consolidados[clave] = {
                ...
                "emp": data.get("emp", 1),
                "empaques_observados": list(
                    data.get("empaques_observados") or [data.get("emp", 1)]
                ),
                "udm_observadas": list(
                    data.get("udm_observadas") or (
                        [data.get("udm_factura", "")] if data.get("udm_factura") else []
                    )
                ),
                "cantidad_facturada": float(data.get("cantidad_facturada", 0)),
    ...
    for _, data in consolidados.items():
        # BASE6-R29.1.5: consolidación matemática correcta.
        # Cada factura ya convirtió su línea a unidades físicas.
        # Aquí SOLO se suman unidades físicas y costos netos.
        stock = float(data["stock"])
        costo_total = float(data["costo_total"])
        unidades_fisicas = float(data.get("unidades_fisicas", stock) or stock)

        # Stock y unidades físicas deben ser equivalentes en el consolidado.
        if abs(unidades_fisicas - stock) > 1e-6:
            unidades_fisicas = stock

        costo_unitario = (
            costo_total / unidades_fisicas
            if unidades_fisicas > 0 else 0.0
        )

        _empaques_validos = []
        for _e in (data.get("empaques_observados") or [data.get("emp", 1)]):
            try:
                _e = max(1, int(float(_e or 1)))
            except Exception:
                _e = 1
            if _e not in _empaques_validos:
                _empaques_validos.append(_e)

        # Este campo es metadata de WilPOS; NO participa en el costo.
        # Si el producto llegó unas veces por caja y otras por unidad,
        # usar 1 para no representar falsamente todo el consolidado como caja.
        empaque_exportar = _empaques_validos[0] if len(_empaques_validos) == 1 else 1
        ...
        filas.append({
            ...
            "Venta Granel": "No",
            "Cantidad Empaque": empaque_exportar,
            "Precio Variable": "No",
            "Descuento %": 0,
            "Descuento Monto": 0,
    ...
    with pd.ExcelWriter(output, engine="openpyxl") as writer:
        ...
        pd.DataFrame({
            "Instrucciones para importar en WilPOS": [
                "La hoja Productos conserva el formato de importación WilPOS.",
                "La hoja Auditoría Costos calcula cada línea de factura por separado.",
                "Cantidad física = Cantidad factura × Cantidad por empaque.",
                "Costo por unidad = Costo neto línea sin ITBIS ÷ Cantidad física.",
                "El costo consolidado = suma de costos netos ÷ suma de unidades físicas.",
...
def _preparar_revision_empaques_modal(validas):
    ...
    for i_fac, (_, _, proveedor, num_fac, _, productos) in enumerate(validas):
        for i_prod, prod in enumerate(productos or []):
            if not isinstance(prod, dict):
                continue
            ...
            filas.append({
                "_id": rid,
                "Factura": str(num_fac),
                "Producto": validado.get("nombre") or validado.get("nombre_original_lectura") or "",
                "Cantidad factura": cant,
                "UDM": (
                    validado.get("purchase_unit")
                    or validado.get("unidad_original")
                    or validado.get("uom")
                    or validado.get("udm")
                    or ""
                ),
...
def _resolver_linea_compra_universal(prod, proveedor=""):
    ...
    # R30.3: marcar costos cuya fuente no tiene suficiente separación
    # frente a otra interpretación monetaria de la misma fila.
    try:
        costo_score = float(p.get("costo_resuelto_score") or 0)
        costo_margen = float(p.get("costo_resuelto_margen") or 0)
    except Exception:
        costo_score, costo_margen = 0.0, 0.0

    # Si no hay score (por rutas históricas/locales), no bloquear.
    if costo_score > 0 and costo_score < 115:
        motivos.append("costo con evidencia insuficiente")
    elif costo_score > 0 and costo_margen < 8:
        motivos.append("costo monetario ambiguo")
    ...
    valido = (
        ...
        and "costo con evidencia insuficiente" not in motivos
        and "costo monetario ambiguo" not in motivos
    )
            ...
            for archivo, firma, proveedor, num_fac, fecha_fac, productos_en_archivo in validas:
                ...
                for p in productos_en_archivo:
                    ...
                    if codigo in st.session_state.inventario_acumulado:
                        ...
                        # Cada origen mantiene su propio empaque.
                        _lista_empaques = st.session_state.inventario_acumulado[codigo].setdefault(
                            "empaques_observados", []
                        )
                        try:
                            ...
                            _emp_nuevo = 1
                        if _emp_nuevo not in _lista_empaques:
                            _lista_empaques.append(_emp_nuevo)

                        _lista_udm = st.session_state.inventario_acumulado[codigo].setdefault(
                            "udm_observadas", []
                        )
                        _udm_nueva = str(
                            p.get("purchase_unit")
                        ...
                        if _udm_nueva and _udm_nueva not in _lista_udm:
                            _lista_udm.append(_udm_nueva)
                    else:
                        ...
                        "costo_original": costo_original,
                        "tasa_usd_dop": float(tasa_usd_dop) if moneda_original == "USD" else None,
                            # Empaque aquí es solo referencia de la primera aparición.
                            # El costo final NO depende de este valor.
                            "emp": p["emp"],
                            "empaques_observados": [int(p["emp"])],
                            "udm_observadas": [str(
                                p.get("purchase_unit")
                                or p.get("unidad_original")
                                or p.get("uom")
...
def _construir_df_auditoria_costos():
    """
    Auditoría POR LÍNEA/FACTURA, antes de la consolidación final.

    No corrige ni sobrescribe silenciosamente los valores.
    Compara:
        cantidad física esperada = cantidad factura × cantidad por empaque
    contra:
        cantidad física usada realmente por el inventario.
    """
    filas = []
    ...
    for codigo, apariciones in (st.session_state.get("origen_productos_facturas", {}) or {}).items():
        for item in apariciones or []:
            ...
            try:
                cantidad_fisica_usada = float(item.get("unidades") or 0)
            except Exception:
                cantidad_fisica_usada = 0.0
            ...
            costo_por_unidad = (
                costo_neto_linea / cantidad_fisica_usada
                if cantidad_fisica_usada > 0 else 0.0
            )

            diferencia_unidades = cantidad_fisica_usada - cantidad_fisica_esperada
            valida = (
                cantidad_factura > 0
                and cantidad_fisica_usada > 0
                and abs(diferencia_unidades) <= 1e-6
            )
            ...
            filas.append({
                ...
                "Cantidad física usada": cantidad_fisica_usada,
...
def _render_auditoria_costo_fisico_ui():
    ...
    alertas_costo = _detectar_costos_atipicos_origen()
    if alertas_costo:
        st.warning(
            f"💲 {len(alertas_costo)} compra(s) del mismo producto tienen un costo físico "
            "muy distinto de otras compras del mismo producto. Revísalas antes de exportar."
        )
        st.dataframe(
            pd.DataFrame(alertas_costo),
            use_container_width=True,
            hide_index=True,
        )

    with st.expander(
        "🧮 Costo por línea: cantidad, empaque y unidad física",
        expanded=False,
    ):
        st.caption(
            "Cantidad física = Cantidad factura × Cantidad por empaque · "
            "Costo por empaque = Costo neto línea ÷ Cantidad factura · "
            "Costo por unidad = Costo neto línea ÷ Cantidad física usada."
        )
        st.dataframe(
            df_auditoria,
            use_container_width=True,
